export default function Footer({ lang = "en" }) {
  const t = {
    en: { features: "Features", chat: "Chat Roulette", minecraft: "Minecraft" },
    ru: { features: "Функции", chat: "Чат‑рулетка", minecraft: "Minecraft" },
  }[lang];
    ...
    <footer className="relative border-t border-white/10 bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-sm text-slate-400 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="#chat" className="hover:text-slate-200 transition-colors">{t.chat}</a>
          <span className="opacity-30">•</span>
          <a href="#resources" className="hover:text-slate-200 transition-colors">{t.minecraft}</a>
        </div>
      </div>
    </footer>
  );
